...
c "mvn clean verify (backend_suite.log)" 'grep -q "^EXIT 0" parity/wave4/coactvwc/backend_suite.log && grep -q "BUILD SUCCESS" parity/wave4/coactvwc/backend_suite.log'
...
c "frontend build"                      '(cd frontend && npm run build > ../parity/wave4/coactvwc/frontend_build.log 2>&1)'
c "frontend karma (ChromeHeadless)"     '(cd frontend && npx ng test --watch=false --browsers=ChromeHeadless > ../parity/wave4/coactvwc/frontend_test.log 2>&1)'
echo "--- DATA/BOUNDARY"
